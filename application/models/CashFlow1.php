...
class CashFlow1 extends CI_Model
{
    ...
    function calculateCumulativeAvailability($year)
    {
        ...
        FROM (SELECT
                ...
                WHERE pay_day >= '$year-01-01'::DATE AND pay_day <= '$year-01-31'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-02-01'::DATE AND pay_day <= '$year-02-28'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-03-01'::DATE AND pay_day <= '$year-03-31'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-04-01'::DATE AND pay_day <= '$year-04-30'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-05-01'::DATE AND pay_day <= '$year-05-31'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-06-01'::DATE AND pay_day <= '$year-06-30'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-07-01'::DATE AND pay_day <= '$year-07-31'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-08-01'::DATE AND pay_day <= '$year-08-31'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-09-01'::DATE AND pay_day <= '$year-09-30'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-10-01'::DATE AND pay_day <= '$year-10-31'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-11-01'::DATE AND pay_day <= '$year-11-30'::DATE) AS bills_to_pay,
        ...
        FROM (SELECT
		        (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                WHERE pay_day >= '$year-12-01'::DATE AND pay_day <= '$year-12-31'::DATE) AS bills_to_pay,
    ...
    function calculateFinalResult($year)
    {
	    ...
	    FROM (
	          SELECT 
                ...
                (SELECT bills.bills_to_receive - bills.bills_to_pay AS cumulative_availability 
                  ...
                  (SELECT
		            (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                      WHERE pay_day >= '$year-01-01'::DATE AND pay_day <= '$year-01-30'::DATE) AS bills_to_pay,

		            (SELECT COALESCE(sum(received_amount), CAST(0 AS money)) FROM bills_to_receive 
                  WHERE receipt_day >= '$year-01-01'::DATE AND receipt_day <= '$year-01-30'::DATE) AS bills_to_receive
	    ...
	    FROM (
	          SELECT 
                ...
                (SELECT bills.bills_to_receive - bills.bills_to_pay AS cumulative_availability 
                  ...
                  (SELECT
		            (SELECT COALESCE(sum(paid_amount), CAST(0 AS money)) FROM bills_to_pay 
                      WHERE pay_day >= '$year-02-01'::DATE AND pay_day <= '$year-01-28'::DATE) AS bills_to_pay,

		            (SELECT COALESCE(sum(received_amount), CAST(0 AS money)) FROM bills_to_receive 
                  WHERE receipt_day >= '$year-02-01'::DATE AND receipt_day <= '$year-01-28'::DATE) AS bills_to_receive
                  )AS bills)) as final")->result();
